...
def get_score(score, bkg, events, siglabel, bkglabel):
    ...
    """
    if bkg == "qcd":
        events[score] = events[f"score_{siglabel}"] / (
            events[f"score_{siglabel}"]
        ...
        events[score] = events[f"score_{siglabel}"] / (
            events[f"score_{siglabel}"] + events[f"score_{bkglabel}"]
        )
...
def get_branches(args, bkg, siglabel, bkglabel):
    ...
    if bkg == "qcd":
        ...
        branches.extend(
            ["label_QCD_b", "label_QCD_bb", "label_QCD_c", "label_QCD_cc", "label_QCD_others"]
        )
        branches.extend(
            [
                "score_label_QCD_b",
...
def main(args):
    ...
    for i, sig in enumerate(signals):
        ...
        if args.mbranch:
            ...
            mask_proc_mh120130 = (
                (events[args.mbranch] >= 120) & (events[args.mbranch] <= 130) & mask_flat
            )
            ...
            # for flat sample
            fp_tp[r"$m_H$:[50-230]"] = get_roc(
                events, score, siglabel, bkglabel, roc_mask=roc_mask, sig_mask=mask_flat
            )
            ptbins = [200, 251, 316, 398, 501, 630, 793, 997, 1255, 1579, 1987, 2500]
            fp_tp[r"$m_H$:[50-230]\times p_T^SM"] = get_roc(
                events,
                score,
                siglabel,
                bkglabel,
            ...
            fp_tp[r"$m_H$:[120,130]\times p_T^SM"] = get_roc(
                events,
                score,
                siglabel,
                bkglabel,
            ...
            # for mh125
            fp_tp[r"$m_H$:125"] = get_roc(
                events, score, siglabel, bkglabel, roc_mask=roc_mask, sig_mask=mask_mh125
            )
            # fp_tp[r'$m_H$:125 PNnonMD'] = get_roc(events, oldpn, siglabel, bkglabel, roc_mask=roc_mask, sig_mask=mask_mh125)

            if args.nprongs:
                fp_tp[r"$m_H$:[50-230] 3prongs"] = get_roc(
                    events,
                    score,
                    siglabel,
                    bkglabel,
                    roc_mask=roc_mask,
                    sig_mask=(mask_flat & (events["fj_nProngs"] == 3)),
                )
                fp_tp[r"$m_H$:[50-230] 4prongs"] = get_roc(
                    events,
                    score,
                    siglabel,
                    bkglabel,
                    roc_mask=roc_mask,
                    sig_mask=(mask_flat & (events["fj_nProngs"] == 4)),
                )
                fp_tp[r"$m_H$:125 3prongs"] = get_roc(
                    events,
                    score,
                    siglabel,
                    bkglabel,
                    roc_mask=roc_mask,
                    sig_mask=(mask_mh125 & (events["fj_nProngs"] == 3)),
                )
                fp_tp[r"$m_H$:125 4prongs"] = get_roc(
                    events,
                    score,
                    siglabel,
                    bkglabel,
                    roc_mask=roc_mask,
                    sig_mask=(mask_mh125 & (events["fj_nProngs"] == 4)),
                )
        else:
            if args.nprongs:
                fp_tp["3prongs"] = get_roc(
                    events,
                    score,
                    siglabel,
                    bkglabel,
                    roc_mask=roc_mask,
                    sig_mask=(events["fj_nProngs"] == 3),
                )
                fp_tp["4prongs"] = get_roc(
                    events,
                    score,
                    siglabel,
                    bkglabel,
                    roc_mask=roc_mask,
                    sig_mask=(events["fj_nProngs"] == 4),
                )
        ...
        bin_ranges = [list(range(300, 1300, 200))]
        ...
        percentiles, cuts = computePercentiles(
            events[score][(events[bkglabel] == 1)].to_numpy(), [0.97, 0.99, 0.995]
        )
        # plot how variables look after cut on tagger score
        plot_var_aftercut(
            odir, hist_features, ["msd"], [bkg], label + "_msdsculpt", cuts, percentiles
        )
    ...
    plot_roc(
        ...
        title=f"ROC Curve of HWW vs {bkglegend}",
        ptcut=ptcut,
        msdcut=msdcut,
    )

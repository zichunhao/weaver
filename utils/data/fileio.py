...
def _read_root(filepath, branches, load_range=None, treename=None):
    with uproot3.open(filepath) as f:
        if treename is None:
            treenames = set(
                [
                    k.decode("utf-8").split(";")[0]
                    for k, v in f.allitems()
...
def _read_parquet(filepath, branches, load_range=None):
    ...
    if load_range is not None:
        df_len = len(df)
        start = math.trunc(load_range[0] * df_len)
        stop = max(start + 1, math.trunc(load_range[1] * df_len))
        outputs = {key: df[key].values[start:stop] for key in branches}
    else:
        outputs = {key: df[key].values for key in branches}
    return outputs
    ...
    for filepath in filelist:
        ...
        try:
            if ext == ".h5":
                ...
                a = _read_root(
                    filepath, branches, load_range=load_range, treename=kwargs.get("treename", None)
                )
            elif ext == ".awkd":
